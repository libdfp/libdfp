...
static void *
gmp_default_realloc (void *old, size_t old_size __attribute__ ((unused)), size_t new_size)
{
  mp_ptr p;
...
static void
gmp_default_free (void *p, size_t size __attribute__ ((unused)))
{
  free (p);
}
...
#define MPZ_REALLOC(z,n) ((int)(n) > (z)->_mp_alloc		\
			  ? mpz_realloc(z,n)			\
			  : (z)->_mp_d)

