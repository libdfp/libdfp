...
int main (void)
{
  ...
  for (d32ptr = printf_d32s; d32ptr->line; d32ptr++)
    {
      memset(format, '\0', CHAR_MAX);
      memset(expect, '\0', CHAR_MAX);
      sprintf(format, "%%Lf %s", d32ptr->format);
      sprintf(expect, "1.000000 %s", d32ptr->expect);
      _PC_P(__FILE__, d32ptr->line, expect, format, f, d32ptr->d);
    }
  ...
  for (d64ptr = printf_d64s; d64ptr->line; d64ptr++)
    {
      ...
      sprintf(format, "%%Lf %s", d32ptr->format);
      sprintf(expect, "1.000000 %s", d32ptr->expect);
      _PC_P(__FILE__, d64ptr->line, expect, format, f, d64ptr->d);
    }
  ...
  for (d128ptr = printf_d128s; d128ptr->line; d128ptr++)
    {
      ...
      sprintf(format, "%%Lf %s", d32ptr->format);
      sprintf(expect, "1.000000 %s", d32ptr->expect);
      _PC_P(__FILE__, d128ptr->line, expect, format, f, d128ptr->d);
    }
